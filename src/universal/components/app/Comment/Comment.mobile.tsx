...
const StyledBox = styled.div`
  ${border('black')};
  margin-left: ${(props: {left: any, size: any}) => props.left};
  padding: 5px 5px;
...
const getGrandParents = ((comments) => {
  const res = []
  Object.entries(comments).forEach(([key, value]: [string, any]) => {
    if (value.depth === 1) {
      res.push(key);
    }
  })
  ...
  ? comments.map((comment, idx) => {
    ...
    return (
      <StyledComment key={idx}>
        <Row>
          <StyledName>
            <StyledBox left='0px' size='fit-content'>
              {comment.user.username}
            </StyledBox>
            <StyledBox left='10px' size='fit-content'>
              {updatedTime}
            </StyledBox>
          </StyledName>
        </Row>
        <Row>
          <StyledBox left='0px' size='100%'>
            {comment.content}
          </StyledBox>
        </Row>
        <StyledVote>
